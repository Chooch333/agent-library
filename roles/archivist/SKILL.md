...
- The brief carries PM intents (`record_pm_item`, `append_pm_assertion`, `set_pm_due`, `change_pm_links`, and from BRIEF_SCHEMA v0.9.0 `record_design_decision`): as of BRIEF_SCHEMA v0.8.0 (BB-2026-09-25-graph-write-protocol) PM intents are filed by the world-graph write protocol, not the Archivist — the filer strips them before this skill runs, so never write `projects/<slug>/pm/workbook.md`; §6–§7 below are kept for history only.
...
**1. The additive rule (governs every step below).**
   ...
   - **`sources` is always a list — never merge a list entry into a mapping.** Some entity types carry another frontmatter field that happens to be a mapping (e.g. `project`'s `lens_sources`, a separate project-only mapping documented in SCHEMA.md — distinct from and unaffected by the generic `sources` list). Before writing a `sources` back-reference: if `sources` is absent, create it as a new list; if it is already a list, append to it; if the existing `sources` value is anything other than a list (a mapping, a string), do **not** write into it — leave it untouched and **surface** the collision instead. This is a stop-and-surface case like any other in this skill, never a merge to attempt (the C-J-108 collision, Build 8.3, 2026-09-13: a list item filed into a mapping under the same `sources` key produced invalid YAML in production).
...
**2. `create_entity`** — write a new entity page.
   ...
   - Record the back-reference: add the `brief_id` to a `sources` entry (e.g. `- "brief: BR-2026-06-03-a7f3"`) so provenance is one hop. Apply the sources-is-always-a-list check from §1 before writing — never merge into an existing non-list `sources` value.
...
- **v0.2.1 (2026-09-13)** — Regression guard (BB-2026-09-11-project-screen-cleanup, Build 8.3, item 0), closing the C-J-108 collision: a `sources` back-reference write now checks the existing value first — absent → create a list; already a list → append; anything else (a mapping, a string) → leave it untouched and **surface**, never merge a list entry into it. Added to §1's additive rule, cross-referenced from §2's `create_entity` back-reference step, and documented as a new Pitfall. No other filing behavior changed.
