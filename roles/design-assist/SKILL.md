...
1. **Intake.** Charles dumps the raw idea, however messy. Capture it verbatim into the artifact before shaping anything. Create the draft Build Brief now. Run the dependency hunt (doctrine rule 7). Also pull the Comms Table's disclosure inbox for this project (`list_judgment_calls`, status `noted`) — the "Calls to review" list, separate from and never gating on blocking questions. For each: read the thread (`build_question_messages`), then `dispose_judgment_call` — `reviewed-agree` if it holds up as-is, or `reviewed-corrected` (log the correcting Decision first, pass its id as `linked_decision`) if it doesn't. Unreviewed items are fine to leave `noted`; nothing here blocks intake. Also pull the **succeeded-unreviewed plans** list for this project — plans with status `succeeded` and `reviewed_at` null (`list_plans`, filtered) — alongside the disclosure inbox; both are session-start pulls, not gates. Also pull **Punch List items with status `needs-brief`** (`list_punch_items`, filtered) — a third session-start pull, alongside the disclosure inbox and the succeeded-unreviewed plans list, not a gate. For each: if it clearly needs a Build Brief, draft and shelve one per the normal Build Brief process below, then `add_punch_note` (author `da`) naming the new Brief's id on the item — this is the only note a DA chat writes on a Punch List item; it never changes the item's status (only the Inspector closes a Punch List item, per PROTOCOL.md's Punch List section). If it doesn't clearly need a brief after a look, leave it for a later session rather than guessing. For each: audit it — verify the executor's claims against live code/data (don't take the report on faith), dispose every disclosure the build posted for that plan (same `dispose_judgment_call` step above), and fill in or fix any missing or weak plain labels (`plain_title`/`plain_summary`) on the plan — a nameless item gets named here at the latest, never left for Charles. Once the audit is clean, call `review_plan`. **Walkthrough visibility rule (mandatory, CB-228).** If the reviewed build has a walkthrough for Charles, the same turn that calls `review_plan` also: (a) updates the walkthrough's header in `docs/review/walkthrough-log.md` from PENDING to READY with the landing time and evidence path — a section prepared ahead of a build must never still say "not merged" after it has; and (b) adds one open next move on the project — `Charles: walk Walkthrough N — checks Build X (<what it covers>), ≈<minutes>. Script: <GitHub link to the log section>. Start at: <preview URL>.` — tagged `walkthrough`, `charles`, `build-X`. The git log stays the single source of the script; Project State carries the pointer, because a walkthrough is work only Charles can do and belongs where his work is listed. The DA completes that next move when Charles reports and logs his findings as WT rows. Unaudited plans are fine to leave for a later session; nothing here blocks intake.
...
10. **Handoff.** The brief passes the completeness gate and ships per PROTOCOL.md, or work continues inline if small. Commit the final brief to the target project's repo (`docs/design/`) if one exists.
   ...
   **Handoff Reference block (mandatory).** The turn that shelves a brief ends with a clearly labeled block Charles can copy into any other chat, containing, in this order: the **build number** (`**Build:** 11.6`, or `unnumbered`), Brief ID, Project State plan title + plan_id + project slug + status, git location (owner/repo/path), and the pasteable prompt — whose first words are that same build number (`Build 11.6 — execute Build Brief BB-…`). The number is what Charles orients by; the ids are for the chats. It also leads the plan's `plain_title`. The block's exact layout and prompt structure follow `references/handoff-reference-template.md` — fill placeholders, never restructure. Shelving without this block is a session defect.
...
- **0.2.15** (2026-09-22) — Step 1's session-start intake gained a third pull: Punch List items in status `needs-brief` (`list_punch_items`), alongside the disclosure inbox and the succeeded-unreviewed plans list. A DA chat that briefs one shelves a Build Brief as usual and adds one `da`-authored note naming the Brief's id (`add_punch_note`) — it never changes the item's status; only the Inspector closes a Punch List item. Per BB-2026-09-21-inspector-repairer Phase 2, step 10.
- **0.2.14** (2026-09-20) — The build number leads the handoff: Step 10's Handoff Reference block opens with `**Build:** N.N` and the pasteable prompt opens with `Build N.N — execute Build Brief …`; unnumbered work says so in the same position rather than dropping the field. Companion PROTOCOL.md rule ("The build number leads the handoff") and `references/handoff-reference-template.md` v1.1.0. Charles, 2026-09-20: the Brief ID, plan id and project slug are for the chats — the build number is the label he orients by, and it was missing from the block.
